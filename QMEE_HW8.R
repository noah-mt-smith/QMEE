...
## BMB: I would probably be careful not to be *too* opinionated in my priors ...
## (i.e. don't make them too narrow, just rule out unrealistic outcomes)
...
test_prior <- function(p) {
  ...
  gg1 <- ggplot(p_df, aes(resource, .prediction)) + geom_jitter() +
      geom_violin(fill = "red", alpha = 0.5) + ylim(0,1) + labs(x = "Resource allocated", 
...
## BMB: hmm, it's not unhappy for me (quick, no reports of divergence) ?
...
## BMB: I think that's fine. We are estimating the uncertainty in the mean of
## the *sample*, not of the data ...
...
## sample fewer draws (for speed - can always go back and remake plots with full sample)
posterior_df <- WLdata_long |> add_predicted_draws(b_custom1, ndraws = 100)

gg2 <- ggplot(posterior_df, aes(resource, .prediction)) +
    geom_jitter(alpha=0.5) + labs(y = "Proportion allocated to winner") +
    stat_sum(data = WLdata_long, aes(y = prop.allocated), col = "skyblue")

print(gg2)

## BMB: not quite sure how we ended up with more than 1e6 samples ...
...
gg_violin <- ggplot(posterior_df, aes(x = resource, y = .prediction)) + geom_violin(fill = "gray") +
    stat_sum(data = WLdata_long, aes(y = prop.allocated), col = "blue")
print(gg_violin)

## BMB: looks like prior might not be capturing the lower tail of money allocation?
...
## BMB: overall this is good. If you want to work through priors for Beta analyses sometime, let me know.
